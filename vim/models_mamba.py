...
def patches_to_image3d(x, stride=(8, 8)):
    # x shape is batch_size x time x lat x lon x channels
    out_channels = x.shape[3] // (stride[0] * stride[1])

    output_h = x.shape[1] * stride[0]
    output_w = x.shape[2] * stride[1]

    x_image = x.permute(0, 3, 1, 2).contiguous()

    x_image = x_image.view(x.shape[0], out_channels, output_h, output_w)
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True, hilbert=True):
        super().__init__()
        ...
        p = int(math.log(self.grid_size[0], 2))
        assert p == math.log(self.grid_size[0], 2), f'expected number of tokens to be a power of two, found grid size: {self.grid_size}'
        hc = HilbertCurve(p, 2)
        n_tokens = self.grid_size[0] ** 2

        self.hilbert_order = torch.zeros(n_tokens, 2)
        self.hilbert_reverse = torch.zeros(n_tokens, 2)

        for i in range(n_tokens):
            self.hilbert_order[i][0] = hc.point_from_distance(i)[0]
            self.hilbert_order[i][1] = hc.point_from_distance(i)[1]

            self.hilbert_reverse[i][1] = hc.point_from_distance(i)[0]
            self.hilbert_reverse[i][0] = hc.point_from_distance(i)[1]

        self.hilbert_order = self.hilbert_order.type(torch.int)
        self.hilbert_reverse = self.hilbert_reverse.type(torch.int)
    ...
    def forward(self, x, reverse=False):
        ...
        x = self.proj(x)
        if self.flatten:
            # apply hilbert flattening here
            if self.hilbert:
                x = x[:, :, self.hilbert_order[:, 0], self.hilbert_order[:, 1]].transpose(1, 2)
            else:
...
class ThreeDPatchEmbed(nn.Module):
    """ 
    3D Image to Patch Embedding
    """
    def __init__(self, img_size=(), patch_size=(), stride=(16), in_chans=3, out_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        super().__init__()
        assert len(img_size) == len(patch_size) == len(stride) == 3, f'expected img_size shape (T, lat, lon) but got: {img_size}'
        self.img_size = img_size
        self.patch_size = patch_size
        ...
        self.grid_size = (img_size[0] // stride[0], img_size[1] // stride[1], img_size[2] // stride[2])
        ...
        # BCTHW
        # it will be three coordinates. the first will repeat at the highest frequency.
        # the second two will be swapped to get different orders. (col and row)
        self.row = torch.stack(
            [
           torch.arange(self.grid_size[0]).repeat(self.grid_size[1] * self.grid_size[2]),
           torch.arange(self.grid_size[1]).repeat_interleave(self.grid_size[0]).repeat(self.grid_size[2]),
           torch.arange(self.grid_size[2]).repeat_interleave(self.grid_size[0] * self.grid_size[1])
            ]
        )
        self.col = torch.stack(
            [
           torch.arange(self.grid_size[0]).repeat(self.grid_size[1] * self.grid_size[2]),
           torch.arange(self.grid_size[1]).repeat_interleave(self.grid_size[0] * self.grid_size[2]),
           torch.arange(self.grid_size[2]).repeat_interleave(self.grid_size[0]).repeat(self.grid_size[1]),
            ]
        )

        tmp = torch.zeros(self.grid_size)

        for i in range(len(self.row)):
            tmp[self.row[i][0], self.row[i][1], self.row[i][2]] = i

        self.col_2_grid = copy(tmp)
        self.to_col = copy(tmp[self.col[0], self.col[1], self.col[2]])

        for i in range(len(self.col)):
            tmp[self.col[i][0], self.col[i][1], self.col[i][2]] = i

        self.row_2_grid = copy(tmp)
        self.to_row = copy(tmp[self.row[0], self.row[1], self.row[2]])

        assert len(self.row[0]) == len(self.col[0]) == self.num_patches
        pad = tuple([int((p - s) // 2) for p, s in zip(patch_size, stride)])

        self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=patch_size, stride=stride, padding=pad, padding_mode='circular')
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()

    def to_row_order(self, toks):
        return toks[self.to_row]

    def to_col_order(self, toks):
        return toks[self.to_col]
    
    def col_to_grid(self, toks):
        tmp = torch.zeros((toks.shape[0], self.grid_size[0], self.grid_size[1], self.grid_size[2], self.out_chans * self.stride[1] * self.stride[2]))
        tmp[:, self.col[0], self.col[1], self.col[2], :] = toks[:, torch.arange(toks.shape[0])]

        return tmp

    def row_to_grid(self, toks):
        tmp = torch.zeros((toks.shape[0], self.grid_size[0], self.grid_size[1], self.grid_size[2], self.out_chans * self.stride[1] * self.stride[2]))
        tmp[:, self.row[0], self.row[1], self.row[2], :] = toks[:, torch.arange(toks.shape[0])]
    ...
    def forward(self, x, transpose=False):
        ...
        x = self.proj(x) 
        if self.flatten:
            # apply cubic flattening here
            if transpose:
                x = x[:, :, self.col[0], self.col[1], self.col[2]].transpose(1, 2)
            else:
                x = x[:, :, self.row[0], self.row[1], self.row[2]].transpose(1, 2)
...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, stride=stride, in_chans=channels, embed_dim=embed_dim, hilbert=hilbert)
...
class WeatherMamba(nn.Module):
    def __init__(self, 
                 img_size=(120, 1280, 640), 
                 patch_size=(1, 14, 14), 
                 stride=(1, 8, 8),
                 depth=24, 
                 embed_dim=192, 
        ...
        self.patch_embed = ThreeDPatchEmbed(
                                        img_size=img_size, patch_size=patch_size, stride=stride, in_chans=channels, out_chans=out_channels, embed_dim=embed_dim
                                     )
        ...
        self.head = nn.Linear(self.num_features, out_channels * self.stride[1] * self.stride[2]) if out_channels > 0 else nn.Identity()
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if not self.if_bidirectional:
            ...
            for i in range(len(self.layers) // 2):
                ...
                # col order
                hidden_states_b, residual_b = self.layers[i * 2 + 1](
                    self.patch_embed.to_col(hidden_states), None if residual == None else self.patch_embed.to_col(residual), inference_params=inference_params
                )
                # sum the hidden states from both orders
                hidden_states = hidden_states_f + self.patch_embed.to_row(hidden_states_b)
                residual = residual_f + self.patch_embed.to_row(residual_b)
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        x = self.patch_embed.row_to_grid(x)

        x = patches_to_image3d(x[:, -1, :, :, :], stride=(self.stride[1], self.stride[2]))
